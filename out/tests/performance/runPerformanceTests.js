...
exports.runPerformanceTests = void 0;
const PerformanceTestSuites_1 = require("./PerformanceTestSuites");
// 命令行参数处理
 ...
 * 运行基准对比测试
 */
async function runBenchmark(testManager, options) {
 ...
 * 持续性能监控
 */
async function runContinuousMonitoring(testManager, options) {
 ...
 * 保存结果到文件
 */
async function saveResults(results, filename, format = 'json') {
    ...
    try {
        if (format === 'json') {
            ...
            const csvData = [
                ...
                ...results.details.map((r) => `${r.testName},${r.success},${r.averageTime},${r.fps},${r.memoryUsage},${r.throughput},${r.latency}`)
            ].join('\n');
...
async function main() {
    ...
    try {
        testManager = new PerformanceTestSuites_1.PerformanceTestManager();
        switch (command) {
            case 'help':
            case '--help':
            case '-h':
