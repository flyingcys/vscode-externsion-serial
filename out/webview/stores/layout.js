...
exports.useLayoutStore = (0, pinia_1.defineStore)('layout', () => {
    // === 状态 ===
    const currentLayoutId = (0, vue_1.ref)('');
    const layouts = (0, vue_1.ref)([]);
    const isEditMode = (0, vue_1.ref)(false);
    const selectedItems = (0, vue_1.ref)([]);
    const clipboard = (0, vue_1.ref)([]);
    // 拖拽状态
    const dragState = (0, vue_1.reactive)({
        isDragging: false,
        draggedItemId: '',
    ...
    // 缩放和网格设置
    const viewSettings = (0, vue_1.reactive)({
        zoom: 1,
        gridSize: 20,
        snapToGrid: true,
        showGrid: false,
    ...
    // === 计算属性 ===
    const currentLayout = (0, vue_1.computed)(() => {
    ...
    const selectedLayout = (0, vue_1.computed)(() => currentLayout.value);
    const canUndo = (0, vue_1.computed)(() => {
        // 实现撤销功能需要历史记录
        return false;
    });
    const canRedo = (0, vue_1.computed)(() => {
        // 实现重做功能需要历史记录  
        return false;
    });
    // === 方法 ===
     ...
     * @returns 新布局ID
     */
    const createLayout = (name, template) => {
        const now = Date.now();
        const id = `layout-${now}`;
        const newLayout = {
            id,
     ...
     * @returns 新布局ID
     */
    const duplicateLayout = (layoutId, newName) => {
     ...
     * @returns 布局项ID
     */
    const addWidget = (widgetId, position) => {
        if (!currentLayout.value) {
     ...
     */
    const selectItems = (itemIds, addToSelection = false) => {
        if (addToSelection) {
     ...
     */
    const paste = () => {
        if (!currentLayout.value || clipboard.value.length === 0) {
            return;
        }
        const now = Date.now();
    ...
    const autoLayout = (type) => {
        if (!currentLayout.value || currentLayout.value.items.length === 0) {
            return;
        }
     ...
     * 网格布局
     */
    const layoutAsGrid = (items, containerWidth, containerHeight, padding) => {
     ...
     * 堆叠布局
     */
    const layoutAsStack = (items, containerWidth, padding) => {
     ...
     * 流式布局
     */
    const layoutAsFlow = (items, containerWidth, padding) => {
    ...
    return {
        // 状态
        currentLayoutId: (0, vue_1.computed)(() => currentLayoutId.value),
        layouts: (0, vue_1.computed)(() => layouts.value),
        isEditMode: (0, vue_1.computed)(() => isEditMode.value),
        selectedItems: (0, vue_1.computed)(() => selectedItems.value),
        dragState: (0, vue_1.computed)(() => dragState),
        viewSettings: (0, vue_1.computed)(() => viewSettings),
        // 计算属性
        currentLayout,
        visibleItems,
        selectedLayout,
...
exports.default = exports.useLayoutStore;
