...
class MachineID {
     ...
     * 获取Windows系统的机器ID
     */
    async getWindowsMachineId() {
        let machineGuid = '';
        let uuid = '';
        try {
            // 获取MachineGuid从注册表
            const { stdout: regOutput } = await execAsync('reg query "HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Cryptography" /v MachineGuid');
        ...
        try {
            // 使用PowerShell获取UUID
            const { stdout: psOutput } = await execAsync('powershell -ExecutionPolicy Bypass -command "(Get-CimInstance -Class Win32_ComputerSystemProduct).UUID"');
            uuid = psOutput.trim();
        }
        catch {
            // 忽略错误
