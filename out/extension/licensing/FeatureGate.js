...
exports.FeatureGate = exports.FallbackBehavior = exports.LicenseType = void 0;
...
(function (FallbackBehavior) {
    ...
    /** 显示但禁用 */
    FallbackBehavior["Disable"] = "disable";
    /** 显示升级提示 */
    FallbackBehavior["ShowUpgrade"] = "show_upgrade";
    /** 使用基础实现 */
    FallbackBehavior["UseBasic"] = "use_basic";
...
class FeatureGate {
    ...
    async requireFeature(featureId, showUpgradePrompt = true) {
        ...
        if (showUpgradePrompt && result.message) {
            ...
            const selection = await vscode.window.showWarningMessage(result.message, upgradeAction, learnMoreAction);
     ...
     */
    isLicenseTypeSufficient(current, required) {
        const levels = {
            [LicenseType.Free]: 0,
            [LicenseType.Pro]: 1,
     ...
     * @param featureId 特性ID
     */
    async showFeatureInfo(featureId) {
        const feature = this.features.get(featureId);
        if (!feature) {
            return;
        }
        ...
        const upgradeAction = '升级许可证';
        const selection = await vscode.window.showInformationMessage(message, { modal: true }, upgradeAction);
        if (selection === upgradeAction) {
            await this.licenseManager.buy();
        }
    }
     ...
     * 显示不可用特性列表
     */
    async showUnavailableFeatures() {
        const unavailable = this.getUnavailableFeatures();
        const items = unavailable.map(({ feature, result }) => ({
