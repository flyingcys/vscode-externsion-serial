...
class ExcelExporter {
    ...
    async exportData(data, filePath) {
        ...
        try {
            ...
            throw new types_1.ExportError(`Excel export failed: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
     * 设置工作簿属性
     ...
     * @returns 添加的记录数
     */
    async addDataRows(worksheet, data, startRow) {
        let recordCount = 0;
     ...
     */
    async addChart(worksheet, chartConfig, recordCount) {
     ...
     * @param depth 深度
     */
    addObjectToSheet(sheet, obj, depth) {
