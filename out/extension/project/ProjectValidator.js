            ...
            for (let groupIndex = 0; groupIndex < project.groups.length; groupIndex++) {
                ...
                if (group) {
                    ...
                    if (!groupValidation.valid) {
                        valid = false;
                        errors.push(...groupValidation.errors.map(err => `Group ${groupIndex + 1} (${group.title || 'unnamed'}): ${err}`));
                    }
            ...
            for (let actionIndex = 0; actionIndex < project.actions.length; actionIndex++) {
                ...
                if (action) {
                    ...
                    if (!actionValidation.valid) {
                        valid = false;
                        errors.push(...actionValidation.errors.map(err => `Action ${actionIndex + 1} (${action.title || 'unnamed'}): ${err}`));
                    }
                    // 额外的动作验证
                    if (!action.title || action.title.trim().length === 0) {
                        valid = false;
