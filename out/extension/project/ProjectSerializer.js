...
class ProjectSerializer {
    ...
    deserialize(json) {
        return {
            ...
            groups: (json.groups || []).map((groupJson) => this.deserializeGroup(groupJson)),
            actions: (json.actions || []).map((actionJson) => this.deserializeAction(actionJson)),
     ...
     * 对应Group::read()
     */
    deserializeGroup(json) {
        return {
            title: json.title || '',
            widget: json.widget || '',
            datasets: (json.datasets || []).map((datasetJson) => this.deserializeDataset(datasetJson))
        };
    }
    /**
     * 序列化数据集
     ...
     * 处理可能的版本差异和格式兼容性
     */
    importFromSerialStudio(jsonString) {
        try {
     ...
     * 处理不同版本之间的格式差异
     */
    normalizeSerialStudioFormat(json) {
        ...
        if (!normalized.frameStart) {
            normalized.frameStart = '$';
        }
        if (!normalized.frameEnd) {
            normalized.frameEnd = ';';
        }
        ...
        normalized.groups = normalized.groups.map((group) => {
            ...
            if (!normalizedGroup.datasets) {
                normalizedGroup.datasets = [];
            }
            // 规范化数据集
            normalizedGroup.datasets = normalizedGroup.datasets.map((dataset) => {
