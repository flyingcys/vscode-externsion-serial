...
import { ProjectConfig, Group, Dataset, Action, ValidationResult } from '../types/ProjectTypes';
...
export class ProjectValidator {
   ...
   * 验证完整项目配置
   */
  public validateProject(project: any): ValidationResult {
    const errors: string[] = [];
    let valid = true;
   ...
   * 验证组群配置
   */
  public validateGroup(group: Group): ValidationResult {
    const errors: string[] = [];
    let valid = true;
   ...
   * 验证数据集配置
   */
  public validateDataset(dataset: Dataset): ValidationResult {
    const errors: string[] = [];
    let valid = true;
   ...
   * 业务逻辑验证
   */
  private validateBusinessLogic(project: ProjectConfig): ValidationResult {
    const errors: string[] = [];
    let valid = true;
   ...
   * 创建项目JSON Schema
   */
  private createProjectSchema(): any {
    return {
      type: 'object',
      properties: {
        title: { type: 'string', minLength: 1 },
        decoder: { type: 'number', enum: [0, 1, 2] }, // PlainText, Hex, Base64
        frameDetection: { type: 'number', enum: [0, 1, 2, 3] }, // 帧检测模式
