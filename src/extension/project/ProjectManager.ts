...
export class ProjectManager extends EventEmitter {
  ...
  private constructor() {
    ...
    this._serializer = new ProjectSerializer();
  ...
  public async askSave(): Promise<boolean> {
    ...
    const result = await vscode.window.showWarningMessage(
      `The project "${this._title}" has unsaved changes.`,
      { modal: true },
      'Save',
      'Don\'t Save',
      'Cancel'
...
`;
   ...
   * 获取下一个数据集索引
   */
  private getNextDatasetIndex(): number {
    if (!this._currentProject) {return 1;}
  ...
  public static resetInstance(): void {
    if (ProjectManager._instance) {
      ...
      ProjectManager._instance = undefined as any;
    }
  }
}