...
export class DataTransformer {
   ...
   * @returns 转换后的数据记录
   */
  private applyTransformationToRecord(record: any[], transformation: DataTransformation): any[] {
   ...
   * @returns 转换后的数据记录
   */
  private convertUnitInRecord(record: any[], transformation: DataTransformation): any[] {
    const { columnIndex, conversionFactor } = transformation.config;
   ...
   * @returns 转换后的数据记录
   */
  private roundPrecisionInRecord(record: any[], transformation: DataTransformation): any[] {
    const { columnIndex, precision } = transformation.config;
   ...
   * @returns 转换后的数据记录
   */
  private formatDateInRecord(record: any[], transformation: DataTransformation): any[] {
    const { columnIndex, format } = transformation.config;
   ...
   * @returns 转换后的数据记录
   */
  private applyCustomFunction(records: any[][], transformation: DataTransformation): any[][] {
   ...
   * @returns 转换后的数据记录
   */
  private applyCustomFunctionToRecord(record: any[], transformation: DataTransformation): any[] {
    const { columnIndex, customFunction } = transformation.config;
   ...
   * @returns 转换配置
   */
  static createCustomFunction(
    columnIndex: number,
    customFunction: (value: any, record: any[], index: number) => any
  ): DataTransformation {
    return {
  ...
  static readonly CONVERSION_FACTORS = {
    ...
    CELSIUS_TO_KELVIN: (celsius: number) => celsius + 273.15,
    KELVIN_TO_CELSIUS: (kelvin: number) => kelvin - 273.15,
