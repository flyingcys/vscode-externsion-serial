...
import { getMemoryManager } from '../../shared/MemoryManager';
...
/**
 * 优化的数据点存储结构
 * 使用固定大小的循环缓冲区，减少内存分配和GC压力
 */
interface OptimizedDataBuffer {
  buffer: Float32Array;  // 使用TypedArray提高性能和内存效率
  timestamps: Float64Array;  // 时间戳单独存储
  size: number;          // 当前缓冲区大小
  head: number;          // 头指针
  tail: number;          // 尾指针
  capacity: number;      // 容量
...
export const useDataStore = defineStore('data', () => {
  ...
  const frameCache = new DataCache<ProcessedFrame>({
    maxSize: 1000,
   ...
   * @returns 优化的数据缓冲区
   */
  const createOptimizedBuffer = (capacity: number = 10000): OptimizedDataBuffer => {
    return {
   ...
   * @param point 数据点
   */
  const addToOptimizedBuffer = (buffer: OptimizedDataBuffer, point: DataPoint): void => {
    // 将数据点添加到缓冲区
   ...
   * @returns 数据点数组
   */
  const getFromOptimizedBuffer = (buffer: OptimizedDataBuffer, maxPoints?: number): DataPoint[] => {
    ...
    if (pointsToGet === 0) {return result;}
    ...
    for (let i = 0; i < pointsToGet; i++) {
      ...
      const dataPoint = objectPoolManager.acquireDataPoint();
   ...
   */
  const releaseDataPoints = (dataPoints: DataPoint[]): void => {
    objectPoolManager.releaseDataPoints(dataPoints);
  };
   ...
   * @param buffer 优化缓冲区
   */
  const clearOptimizedBuffer = (buffer: OptimizedDataBuffer): void => {
    buffer.size = 0;
    buffer.head = 0;
    buffer.tail = 0;
    buffer.isFull = false;
   ...
   * @param timestamp 时间戳
   */
  const processGroup = (group: Group, timestamp: number) => {
    // 更新或创建widget
   ...
   * @param point 数据点
   */
  const addDataPoint = (datasetId: string, point: DataPoint) => {
   ...
   * @returns 数据点数组
   */
  const getDataPoints = (datasetId: string, maxPoints?: number): DataPoint[] => {
   ...
   * @param count 数量
   */
  const getLatestDataPoints = (datasetId: string, count = 100): DataPoint[] => {
    const buffer = state.dataBuffer.get(datasetId);
    if (!buffer) {return [];}
   ...
   * @param endTime 结束时间
   */
  const getDataPointsInRange = (datasetId: string, startTime: number, endTime: number): DataPoint[] => {
    const buffer = state.dataBuffer.get(datasetId);
    if (!buffer) {return [];}
    ...
    // 释放不需要的数据点回对象池
    const pointsToRelease = allPoints.filter(point => 
      point.timestamp < startTime || point.timestamp > endTime
    );
   ...
   * @param active 是否激活
   */
  const setWidgetActive = (widgetId: string, active: boolean) => {
  ...
  const exportData = (options: {
    ...
    const data: any = {};
    const pointsToRelease: DataPoint[] = [];
