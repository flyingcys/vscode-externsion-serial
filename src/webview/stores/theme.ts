...
export const useThemeStore = defineStore('theme', () => {
  // === 状态 ===
  const currentThemeType = ref<ThemeType>('auto');
   ...
   */
  const importTheme = (themeJson: string): void => {
    try {
      const theme: ColorTheme = JSON.parse(themeJson);
   ...
   * @returns 是否有效
   */
  const validateTheme = (theme: any): theme is ColorTheme => {
    return theme &&
  ...
  return {
    // 状态
    currentThemeType: computed(() => currentThemeType.value),
