...
export class PerformanceBenchmark {
   ...
   * 运行3D渲染性能测试
   */
  async benchmark3DRendering(): Promise<number> {
   ...
   * 验证性能目标
   */
  private validatePerformanceTargets(): Array<{ name: string, passed: boolean, detail: string }> {
    console.log('\n📋 验证性能目标...');
...
// 自动化测试函数
export async function runAutomatedBenchmark(): Promise<void> {
  try {
...
if (typeof window !== 'undefined') {
  // 将测试函数暴露到全局作用域
  (window as any).runPerformanceBenchmark = runAutomatedBenchmark;
