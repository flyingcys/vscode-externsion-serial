...
class JavaScriptSandbox {
  ...
  private initializeContext(): void {
    this.context = {
      ...
      // 控制台输出（仅在调试模式下）
      console: {
        log: (...args: any[]) => console.log('[Frame Parser]', ...args),
        warn: (...args: any[]) => console.warn('[Frame Parser]', ...args),
        error: (...args: any[]) => console.error('[Frame Parser]', ...args)
      },
   ...
   * 当没有用户脚本时使用
   */
  private defaultParse(frameData: string | Uint8Array): any[] {
    try {
