...
import PerformanceTestFramework, { TestCase, PerformanceTestConfig } from './PerformanceTestFramework';
import HighFrequencyRenderer from '../../shared/HighFrequencyRenderer';
import { VirtualizationManager } from '../../shared/VirtualizationManager';
...
class TestDataGenerator {
   ...
   * 生成模拟串口数据
   */
  static generateSerialData(size: number): Buffer[] {
    const frames: Buffer[] = [];
   ...
   * 生成图表数据点
   */
  static generatePlotData(pointCount: number, seriesCount: number = 1): any[] {
    const datasets = [];
   ...
   * 生成大量表格数据
   */
  static generateTableData(rowCount: number): any[] {
    const rows = [];
 ...
 * 数据处理性能测试
 */
export class DataProcessingTests {
...

      {
        ...
        test: async () => {
          ...
          return {
            processedBytes,
            processingTime,
            dataRate: processedBytes / (processingTime / 1000), // bytes/second
 ...
 * 内存管理性能测试
 */
export class MemoryManagementTests {
  static getTests(): TestCase[] {
    return [
      {
        ...
        test: async () => {
          ...
          objectPoolManager.releaseDataPoints(objects);
...

      {
        ...
        setup: async () => {
          memoryMonitor.startMonitoring(1000, 5000);
        },
        test: async () => {
 ...
 * 渲染性能测试
 */
export class RenderingPerformanceTests {
  static getTests(): TestCase[] {
    return [
      {
...

      {
        ...
        test: async () => {
          ...
          return {
            totalTime,
            totalPointsRendered,
            renderThroughput: totalPointsRendered / (totalTime / 1000), // points/second
 ...
 * 虚拟化性能测试
 */
export class VirtualizationTests {
  static getTests(): TestCase[] {
    return [
      {
        ...
        setup: async () => {
          // 初始化虚拟化管理器
          VirtualizationManager.getInstance();
        },
        test: async () => {
          const vm = VirtualizationManager.getInstance();
          ...
          for (let scrollTop = 0; scrollTop < 50000; scrollTop += 500) {
            const visibleRange = vm.calculateVisibleRange(
              'test-virtual-list',
              scrollTop,
              400,
              testData.length
...
export class PerformanceTestManager {
   ...
   * 运行特定类别的测试
   */
  async runCategory(category: 'data' | 'memory' | 'rendering' | 'virtualization'): Promise<void> {
    const categoryTests = {
